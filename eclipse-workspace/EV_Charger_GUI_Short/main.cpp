...
int main(int argc, char** argv)
{
	...
	plus_button->on_click([&green_wire_image,&remaining_time_text,&blinking_green_wire_timer,&battery_progress_bar, &percentage_charged_text, &remaining_time_value,&elapsed_time_text,&delivered_energy_text,&battery_fully_charged_text,&screen_saver_timer](egt::Event &)
	{
		screen_saver_timer.stop();
		screen_saver_timer.start();
	...
	// What does the script does when the screen_saver_timer reaches 10 seconds : It means that after 10 seconds withtout interruption on the main screen, the application switches from the main_screen to the screen_saver
